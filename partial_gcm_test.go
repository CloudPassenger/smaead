...
func TestAESGCM(t *testing.T) {
	for i, test := range aesGCMTests {
		...
		ct2 := ct[:len(ct)-tagSize]
		if len(ct) > 32 {
			ct2 = ct[:16]
		}
		pt2 := plaintext[:len(ct2)]
		ppt, err := aesgcm.OpenWithoutCheck(nil, nonce, ct2)
		if err != nil {
			t.Errorf("#%d: OpenWithoutCheck failed", i)
			continue
		}
		if !bytes.Equal(pt2, ppt) {
			t.Errorf("#%d: plaintext's don't match: got %x vs %x", i, pt2, ppt)
			continue
		}
		if len(ad) > 0 {
			ad[0] ^= 0x80
